(function(){
    ...
    function readData(toLog) {
        ...
        html+='<div class="h">'+
                    '<h2>'+
                        '<img class="mod-icon" title="来疯直播" src="http://static.youku.com/ddshow/img/static/image/lf-logo.png">'+
                        '<a target="_blank" href="http://cps.laifeng.com/redirect.html?id=00014089&url=http%3A%2F%2Fwww.laifeng.com%2F">来疯直播</a>'+
                    '</h2>'+
                '</div>';
    ...
    function loadConnect(DATA,toLog) {
        ...
        for(var i=0;i<Math.min(DATA.length,6);i++) {
                ...
                if(data.status==1 && !data.livehouse){
                    ...
                    _html+='<a target="_blank" style="color:#909090;" href="'+data.roomUrl+'" '+_log+'>查看详情</a>';
                }
                _html+='</span></li></ul>';
                _html+='</div></div>';
            }
